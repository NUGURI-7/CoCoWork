...
WITH nearest AS (
    SELECT e.paragraph_id, e.document_id, e.text AS chunk_text,
...
dedup AS (
    SELECT DISTINCT ON (n.paragraph_id)
        n.paragraph_id, n.document_id, n.chunk_text, n.distance
    FROM nearest n
    ORDER BY n.paragraph_id, n.distance
)

SELECT d.paragraph_id, d.document_id, doc.name AS doc_name,
    p.content, d.chunk_text, d.distance
FROM dedup d
