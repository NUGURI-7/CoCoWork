...
#
# 曾随「切块降到 128 token」一并抬到 12（块数涨 2.3 倍、去重后段落数会掉到四成），
# 切块粒度判退后一并退回。实测这个块密度下 5 与 12 分数一字不差
# （2026-08-15 医疗基准：两次均为 recall@10 0.7990 / mrr@10 0.6778），
# 故取更省的一档。**真要重调，得先有块密度显著变高的场景。**
#
# **这个常量整个依附于「top_k = 返回段落数」这一前提** —— 它折算的就是
# 「多个子块塌成一个段」的名额损耗。业界通行的是 top_k = 子块数（截断发生在
# 去重之前），那样压根不需要折算，本常量也就不存在。该前提存疑，
# 见 issues/014-topk-semantics.md。
CANDIDATE_FACTOR = 5
