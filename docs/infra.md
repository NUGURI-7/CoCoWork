...
> 开发机内存/磁盘/swap 于 2026-07-25 复测更新。
...
| 内存 | 2G + **swap 2G**（`/swapfile`，2026-07-19 之后补加） | ~1G（961M），swap 2G |
...
| 容器 | 镜像 | 端口绑定 | 说明 |
|---|---|---|---|
| 1Panel-postgresql-1JEO | pgvector/pgvector:pg18 | 0.0.0.0:5432 | **开发主 PG**（1Panel 管理），DaisyWind 与 CoCoWork 共用、仅库名不同（本项目 = `cocowork`）；PG 18.4 + pgvector 0.8.5（**镜像自带**，2026-07-25 从官方 `postgres:18.3-alpine` 换来——原先 pgvector 是手动装进容器的，容器一重建就丢）；**RAG 基准数据（医疗/电商）都在这** |
...
1. **开发机仅 2G 内存**（2026-07-25 实测：used 653M / available 1.1G，磁盘余 35G）。**已补 2G swap**，故内存尖峰不再是「被 OOM killer 直接杀掉」，而是变慢 —— 原「swap=0」的风险条目已失效。仍别调大 PG 的 `maintenance_work_mem`；大批量建索引避开其他负载。
...
6. **PG 版本错位**：开发 18.4 vs 生产 16。SQL 特性以 **16 为下限**写，避免用到 17+ 才有的语法。
