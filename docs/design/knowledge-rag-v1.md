...
- [x] **片4 文档上传**（拆 2 小片）：
  ...
  - [x] **4b 上传端点 + Document CRUD**：R2 走 presign/confirm 三段式、Local 走 passthrough multipart；列表/删除/下载链接
- [x] **片5 处理管线**：`process_document()` 解析→切段→切块→embed（手动触发，`BackgroundTasks` 跑）
- [x] **片6 检索 + 命中测试**：检索 service（`::vector(dims)` cast + `DISTINCT ON` 按段去重）+ 命中测试端点
