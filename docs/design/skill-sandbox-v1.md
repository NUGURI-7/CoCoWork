...
1. **产物引用链路**（决策 14a 改判后的新形态，2026-07-29 定）：产物清单渲染进历史消息 +
   `fetch` 工具（决策 24）+ 前端拖引用与 `artifact_ref` block（决策 25）+
   local driver 的 `/workspace` 改成每轮清空。
2. **沙箱那批纯逻辑单元测试**（layout / `_pack_tar` / `_unpack_single` / 交付区清单解析 /
   降级路径等，都不需要 docker 与 DB）。
3. **用户上传**（zip 层把关 + 对象存储 + CRUD + 前端）—— 本轮不做。
