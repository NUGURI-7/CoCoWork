...
| 样本 | 形态 | 结果 |
|---|---|---|
| 合成测试文档（4 页，Chrome 打印，附答案表） | 单栏 | **全对**：三级标题、页眉页脚过滤、表格独立成块、引注块不误判、跨页合并 |
| 《复合材料学报》（22 页，知网） | **双栏** | 不崩、真章节标题抓得到，但双栏交错导致文本乱序 → **结果不可用**，与「双栏交给云端」的既定判断一致 |
| 学位论文（54 页） | 单栏 | **可用**：40+ 章节标题全抓到、层级关系正确；检出第 2-8 页共 7 页无文本层 |
...
1. **目录页应整页丢弃** —— 目录行现在被切成段、进库、算向量、参与检索。用户搜「第一章 绪论」会命中目录里的 `第一章 绪论 ....... 1` 而非正文。特征明确：一页里大半行以 `......` + 页码结尾。
2. **标题链被 front matter 污染** —— 学位论文封面字号最大（42pt/24pt），被判成 H1/H2 且永不出栈，导致所有段的标题链形如 `( ) > 研究 > 目 录 > 第三章 …`。两个方向：识别并跳过封面/目录，或标题链只保留最后 3 级。
3. **无框线表格（三线表）检测不到** —— `find_tables()` 靠框线，学位论文标准的三线表只有上下横线。54 页论文只识别出 1 个表。
4. **混合 PDF 按页路由** —— 现在部分页无文本层只打 warning、内容静默丢失；第 4 步接云端后应把这些页单独送 OCR，按页码拼回。
5. **页眉与正文同高会被合并成一行** —— `_LINE_TOLERANCE=2.0` 只看 y 不看 x，Chrome 打印的文档中页眉（top=80.2）与正文（top=79.5）被拼成一行乱码。真实 PDF 页眉有独立页边距，暂未触发。
6. **`PdfParser` 纯函数单测未补** —— `_classify_sizes` / `_merge_paragraphs` / `_running_heads` / `_join_continuation` / `_table_to_markdown` 都可手编数据测；需真 PDF 的部分不进 pytest（语料在 gitignored 的 `data/` 下）。
