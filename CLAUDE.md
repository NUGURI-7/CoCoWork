...
8. **`记笔记` 触发词**：当我说「记笔记」时，把当前对话里值得沉淀的概念 / 决策 / 踩坑写进学习笔记。笔记模块固定在项目根目录下的 `notes/`（`notes/backend/<模块>/` 或 `notes/frontend/<模块>/`，每个模块含 `notes.md` + `qa.md`，遵循既有 frontmatter + TL;DR + 编号要点 / QA 风格），**不用每次现找目录**。写之前**先列出「打算记哪些条目」让我审批，审批通过再落盘**，不要直接写。优先追加到已有模块笔记、避免重复，不轻易新建目录。
9. **代码默认生产级最终版**：每次落盘的首版即架构 / 命名 / 扩展性 / 类型 / 异常一次到位；不分"先简后繁"、不写阶段化占位（P0/P1、# TODO 这版先这样）。小步迭代是任务分段，不是代码风格分段。

## 代码分工（谁动文件）

代码改动分两种产出形态：

- **「Claude 写」** = Claude 直接用工具落盘（Write/Edit）。
- **「用户写」** = Claude 在对话里给出**完整、可直接粘贴**的代码，由用户手动敲/贴进文件；Claude **不落盘**。

**后端**

- **用户写**：架构、`service` 层、`model` 层、分层设计、Tortoise **数据库迁移**，以及任何需要做设计取舍的部分 —— Claude 给完整版本代码，用户来写。
- **Claude 写**：`schema` 层、`route` / API 端点层等模板化、无设计含量的部分，直接落盘。**例外**：需要专门设计的端点归用户写。

**前端**

- **Claude 写**：默认前端都由 Claude 直接落盘。
- **用户写**：有设计、有架构、非模板的部分，由用户当场指认。Claude **有义务主动提醒**「这块可能属于需要你写的部分」，交由用户决定，不要默认落盘。

拿不准某块归谁时，先问，不要默认落盘。

### 给代码的方式（不给孤立函数）

给我看代码时按「我坐在编辑器前怎么操作」来写，不要甩一段孤立函数让我自己找位置：

1. 先说**打开哪个文件、改哪一段**（带行号锚点）。
2. 已有文件一律给「**原来长这样 → 改成这样**」，前后留几行原代码当路标；只有新增文件才给完整全文。
3. 说清**为什么改这里**，以及它跟谁有关系（谁调它、参数从哪来、改完谁受影响）。

目的：即便代码由 Claude 落盘，我也要清楚它长在哪、怎么串起来的 ——
我是这个项目唯一的维护者，出了 bug 得自己知道去哪儿看。
