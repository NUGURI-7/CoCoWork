...
4. 贴完代码，再用**自然语言把这段逻辑讲一遍** —— 按执行顺序说清每一步在干
   什么、失败会怎样、为什么是这个顺序。**讲在对话里，不许塞进代码注释**：
   注释密度保持原样，讲述是给我看的、不是给代码看的。

   理由：我现在看不下去纯代码细节了。第 1-3 条解决「这段代码在哪」，
   第 4 条解决「这段代码在干嘛」—— 后者才是我真正在审的那一份。
